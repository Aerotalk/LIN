...
export default function LoanComparisonCalc() {
  ...
  const handleUpdateLoan = (id: number, key: keyof LoanData, value: number) => {
    setLoans((prev) =>
      prev.map((loan) =>
        loan.id === id ? (loan[key] !== value ? { ...loan, [key]: value } : loan) : loan
      )
    );
  };
  ...
  return (
    <section className="w-full max-w-7xl mx-auto py-8 px-4 md:px-8 lg:px-16 mt-24">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {computedLoans.map((loan) => (
          <div key={loan.id} className="relative bg-white rounded-xl border p-6 shadow-sm">
            ...
            <div>
              <div className="flex justify-between items-center mb-2">
                ...
                <Input
                  ...
                  value={loan.tenure}
                  onChange={(e) =>
                    handleUpdateLoan(loan.id, "tenure", Number(e.target.value))
                  }
                />
              </div>
              <Slider
      ...
      {/* Comparison Table */}
      <div className="mt-10 bg-red-500 rounded-xl text-white overflow-x-auto">
